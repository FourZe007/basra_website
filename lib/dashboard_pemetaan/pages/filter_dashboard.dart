...
import 'package:stsj/dashboard_pemetaan/pages/dashboard_pemetaan.dart';
...
class _FilterDashboardState extends State<FilterDashboard> {
  ...
  void find() async {
    try {
      ...
      List<GeoHD> listData = await ApiPemetaan.getPemetaanAktivitas(GlobalVar.username, cabang, area, tgl1, tgl2, '01');
      if (listData.isEmpty) {
        throw ('Hasil Tidak Ditemukan');
      } else {
        if (mounted) Navigator.pop(context);
        if (mounted) Navigator.push(context, MaterialPageRoute(builder: (context) => DashboardPemetaan(listData, tgl1, tgl2)));
      }
    } catch (e) {
